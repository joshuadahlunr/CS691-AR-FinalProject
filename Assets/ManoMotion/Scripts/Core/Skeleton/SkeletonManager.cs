...
namespace ManoMotion
{
    ...
    public class SkeletonManager : MonoBehaviour
    {
        ...
        public Vector3 GetCenterPosition(int handIndex, params int[] joints)
        {
            List<GameObject> skeletonJoints = SkeletonManager.Instance.GetJoints(handIndex);
            Vector3 position = new Vector3();

            for (int i = 0; i < joints.Length; i++)
                position += skeletonJoints[joints[i]].transform.position;

            position /= joints.Length;
